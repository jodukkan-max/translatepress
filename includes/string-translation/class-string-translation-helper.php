...
class TRP_String_Translation_Helper {
	...
	public function get_originals_results( $type, $original_table, $original_meta_table, $get_table_name_func, $filters ) {
		...
		if ( ! empty( $sanitized_args['s'] ) ) {
            ...
            // Build a prepared LIKE value from the raw search term. Escaping is
            // handled by $wpdb->prepare() at execution time, so keep the term raw.
            ...
            foreach ( $translation_languages as $language ){
                ...
                // Use exact match or partial match based on quotes. Placeholders
                // are filled by $wpdb->prepare() at execution time.
                if ( $is_exact_match ) {
                    $language_clauses = [ "$language.translated = %s" ];
                    $prepare_args[]   = $search_term;
                } else {
                    $language_clauses = [ "$language.translated LIKE %s" ];
            ...
            if ( $is_exact_match ) {
                $where_clauses[] = "(original_strings.original = %s )";
                $prepare_args[]  = $search_term;
            } else {
                $where_clauses[] = "(original_strings.original LIKE %s )";
                $prepare_args[]  = $search_term_like;
            }
		}

		if ( ! empty( $sanitized_args['domain'] ) ) {
			$domain_like      = '%' . $wpdb->esc_like( $sanitized_args['domain'] ) . '%';
			$where_clauses[]  = "(original_strings.domain LIKE %s )";
			$prepare_args[]   = $domain_like;
		}
        ...
        $prepared_counting_query = ! empty( $prepare_args ) ? $wpdb->prepare( $counting_query, $prepare_args ) : $counting_query;
        $total_item_count = $wpdb->get_var( $prepared_counting_query );
		$original_ids     = array();
		$originals        = array();
		if ( $total_item_count > 0 ) {
			...
			$prepared_results_query = ! empty( $prepare_args ) ? $wpdb->prepare( $results_query, $prepare_args ) : $results_query;
			$originals     = $wpdb->get_results( $prepared_results_query, OBJECT_K );
